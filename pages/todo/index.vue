<template>
  <div class="bg-[#F7F7F7] h-screen relative">
    <div class="mx-auto w-157 relative">
      <div class="font-bold pt-10 text-[30px] text-center">TODO LIST</div>

      <div class="flex mt-4.5 w-full gap-4 justify-between items-center">
        <div class="relative flex items-center grow">
          <input
            type="text"
            placeholder="Search Note..."
            class="h-full py-2 px-4 w-full rounded-[5px] border border-solid border-[#6C63FF]"
          />
          <div class="absolute right-4 cursor-pointer">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="21"
              height="22"
              viewBox="0 0 21 22"
              fill="none"
            >
              <path
                d="M20.7773 20.184L15.9056 15.312H15.8531C17.3547 13.5415 18.1136 11.2588 17.9709 8.94156C17.8282 6.62433 16.7951 4.45202 15.0876 2.87905C13.3801 1.30608 11.1306 0.454303 8.80958 0.501892C6.48855 0.549481 4.27583 1.49275 2.63427 3.13439C0.992706 4.77602 0.0494786 6.98885 0.00189181 9.30999C-0.045695 11.6311 0.806045 13.8808 2.37894 15.5883C3.95184 17.2958 6.12404 18.329 8.44117 18.4717C10.7583 18.6144 13.0408 17.8555 14.8113 16.3539C14.8113 16.3539 14.8113 16.3914 14.8113 16.4063L19.6831 21.2783C19.7527 21.3485 19.8356 21.4043 19.927 21.4424C20.0183 21.4804 20.1163 21.5 20.2152 21.5C20.3141 21.5 20.4121 21.4804 20.5034 21.4424C20.5948 21.4043 20.6777 21.3485 20.7473 21.2783C20.8242 21.2103 20.8862 21.1272 20.9296 21.0342C20.9731 20.9412 20.9969 20.8402 20.9997 20.7376C21.0025 20.635 20.9842 20.533 20.946 20.4377C20.9077 20.3425 20.8503 20.2561 20.7773 20.184ZM9.00276 16.9685C7.5204 16.9685 6.07133 16.5289 4.83879 15.7053C3.60625 14.8817 2.64561 13.7111 2.07833 12.3415C1.51106 10.9719 1.36263 9.46488 1.65183 8.01094C1.94102 6.55699 2.65485 5.22146 3.70303 4.17322C4.75122 3.12499 6.08669 2.41113 7.54057 2.12192C8.99445 1.83272 10.5014 1.98115 11.871 2.54845C13.2405 3.11575 14.411 4.07644 15.2346 5.30904C16.0581 6.54163 16.4977 7.99077 16.4977 9.4732C16.4977 10.4575 16.3038 11.4322 15.9272 12.3415C15.5505 13.2509 14.9985 14.0772 14.3025 14.7732C13.6065 15.4692 12.7803 16.0213 11.871 16.3979C10.9616 16.7746 9.98701 16.9685 9.00276 16.9685Z"
                fill="#6C63FF"
              />
            </svg>
          </div>
        </div>

        <select
          name=""
          id=""
          class="bg-[#6C63FF] rounded-[5px] p-2 cursor-pointer text-white"
        >
          <option value="">ALL</option>
          <option value="">Task 1</option>
        </select>

        <div class="bg-[#6C63FF] rounded-md cursor-pointer">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="38"
            height="38"
            viewBox="0 0 38 38"
            fill="none"
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M19.1249 8.5488C19.3387 8.91735 19.321 9.3762 19.0791 9.72705C18.3455 10.7915 17.916 12.0806 17.916 13.4733C17.916 17.1243 20.8757 20.084 24.5266 20.084C25.9194 20.084 27.2085 19.6545 28.2729 18.9208C28.6238 18.6791 29.0826 18.6613 29.4512 18.8751C29.8197 19.089 30.0319 19.4962 29.9961 19.9208C29.5191 25.567 24.7867 30 19.0178 30C12.9328 30 8 25.0672 8 18.9822C8 13.2133 12.433 8.48087 18.0792 8.00392C18.5038 7.96806 18.911 8.18024 19.1249 8.5488ZM16.1799 10.6346C12.7045 11.8157 10.2036 15.1073 10.2036 18.9822C10.2036 23.8502 14.1498 27.7964 19.0178 27.7964C22.8927 27.7964 26.1843 25.2955 27.3654 21.8202C26.4741 22.1232 25.5191 22.2875 24.5266 22.2875C19.6587 22.2875 15.7124 18.3413 15.7124 13.4733C15.7124 12.4809 15.8768 11.5258 16.1799 10.6346Z"
              fill="#F7F7F7"
            />
          </svg>
        </div>

        <button
          class="bg-[#6C63FF] text-2xl w-10 h-10 rounded-full flex items-center justify-center cursor-pointer text-white"
          @click="toggleItem()"
        >
          +
        </button>
      </div>

      <div class="divide-y divide-indigo-400">
        <div
          class="flex mt-3 mx-28.75 pb-3 justify-between items-center"
          v-for="(item, index) in items"
          :key="index"
        >
          <label
            class="flex gap-4.25 text-[20px] font-medium cursor-pointer items-center"
          >
            <input type="checkbox" class="w-6.5 h-6.5" />
            <div v-if="!isEditing">{{ item.text }}</div>
            <input v-else type="text" v-model="item.text" />
          </label>

          <div class="flex gap-2.5">
            <div class="cursor-pointer" @click="toggleEditItem()">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
                height="18"
                viewBox="0 0 18 18"
                fill="none"
              >
                <path
                  d="M8.67272 5.99106L2 12.6637V16H5.33636L12.0091 9.32736M8.67272 5.99106L11.0654 3.59837L11.0669 3.59695C11.3962 3.26759 11.5612 3.10261 11.7514 3.04082C11.9189 2.98639 12.0993 2.98639 12.2669 3.04082C12.4569 3.10257 12.6217 3.26735 12.9506 3.59625L14.4018 5.04738C14.7321 5.37769 14.8973 5.54292 14.9592 5.73337C15.0136 5.90088 15.0136 6.08133 14.9592 6.24885C14.8974 6.43916 14.7324 6.60414 14.4025 6.93398L14.4018 6.93468L12.0091 9.32736M8.67272 5.99106L12.0091 9.32736"
                  stroke="#CDCDCD"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
              </svg>
            </div>

            <div class="cursor-pointer" @click="toggleDeleteItem()">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
                height="18"
                viewBox="0 0 18 18"
                fill="none"
              >
                <path
                  d="M3.87463 7.61505C3.80761 6.74386 4.49644 6 5.3702 6H12.6305C13.5044 6 14.1932 6.74385 14.1262 7.61505L13.6069 14.365C13.5468 15.1465 12.8951 15.75 12.1113 15.75H5.88943C5.10563 15.75 4.45397 15.1465 4.39385 14.365L3.87463 7.61505Z"
                  stroke="#CDCDCD"
                />
                <path
                  d="M14.625 3.75H3.375"
                  stroke="#CDCDCD"
                  stroke-linecap="round"
                />
                <path
                  d="M7.5 2.25C7.5 1.83579 7.83577 1.5 8.25 1.5H9.75C10.1642 1.5 10.5 1.83579 10.5 2.25V3.75H7.5V2.25Z"
                  stroke="#CDCDCD"
                />
                <path
                  d="M10.5 9V12.75"
                  stroke="#CDCDCD"
                  stroke-linecap="round"
                />
                <path
                  d="M7.5 9V12.75"
                  stroke="#CDCDCD"
                  stroke-linecap="round"
                />
              </svg>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="absolute inset-0 bg-[#252525B2]" v-if="boxItem">
      <div
        class="absolute bg-white rounded-[16px] py-4.5 px-7.5 flex flex-col gap-32 w-125 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
      >
        <input
          v-model="newItem"
          type="text"
          placeholder="Input your note..."
          class="h-full py-2 px-4 w-full rounded-[5px] border border-solid border-[#6C63FF]"
        />
        <div class="flex justify-between">
          <button
            class="border border-[#6C63FF] text-[#6C63FF] rounded-[5px] py-2.5 px-5.5 cursor-pointer"
            @click="toggleItem()"
          >
            CANCLE
          </button>
          <button
            class="text-white bg-[#6C63FF] rounded-[5px] py-2.5 px-5.5 cursor-pointer"
            @click="toggleAddItem()"
          >
            APPLY
          </button>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { useLocalStorage } from "@vueuse/core";
const items = useLocalStorage("items-local-save", []);

const newItem = ref("");

const boxItem = ref(false);

const toggleItem = () => {
  boxItem.value = !boxItem.value;
};

const toggleAddItem = () => {
  items.value.push({ text: newItem.value, isEditing: false });
  newItem.value = "";
  boxItem.value = false;
};

const toggleEditItem = () => {
  isEditing.value = true;
};

const toggleDeleteItem = () => {};
</script>

<style lang="scss" scoped></style>
